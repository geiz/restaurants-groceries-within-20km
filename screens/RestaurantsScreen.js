...
export default class RestaurantsScreen extends React.Component {
  ...
  async _getResturantsFromApiAsync(latitude, longitude) {
    try {
      const response = await fetch(`https://api.yelp.com/v3/businesses/search?term=resturants&latitude=${latitude}&longitude=${longitude}&radius=20000&sort_by=distance`, {
      method: 'GET',
      headers: {
